...
class AudioDatasetTamil(Dataset):
    ...
    def __getitem__(self, index):        
        ...
        Tamwaveform, s1 = load_audio_new(path=TAMaudio_path.strip(), duration=self.duration,sample_rate=48000,normalization=False)#sr is 44100
        Engwaveform, s2 = load_audio_new(path=ENGaudio_path.strip(), duration=self.duration,sample_rate=48000,normalization=False)
        if self.tag == "spectrogram":
            ...
            return TAMspectrogram, ENGspectrogram,Engwaveform,self.Text[index]
            
        elif self.tag == "melspectrogram":
            transform = torchaudio.transforms.MelSpectrogram( n_fft = 510,hop_length = 272,n_mels=80)
            TAMmelSpectrogram = torch.log10(torch.squeeze(transform(Tamwaveform))+1e-3)
            ENGmelSpectrogram = torch.log10(torch.squeeze(transform(Engwaveform))+1e-3)
            return F.pad((TAMmelSpectrogram),(1,1)), F.pad((ENGmelSpectrogram),(1,1)),Engwaveform,self.Text[index]
